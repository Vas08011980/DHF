...
```
```{r}
DEPDATA<-DASSTotal[c("Age_W1", "Yearsofplay_W1", "IMTotal", "IDTotal", "COMPTotal", "DEPTot")]
DEPDATA$DEPTot=as_factor(DEPDATA$DEPTot)
describe(DEPDATA)
```
...
DEPDATAM <- SMOTE(DEPTot ~ ., DEPDATA, perc.over = 1610, perc.under = 107)
table(DEPDATAM$DEPTot)
...
set.seed(123)
data_split <- initial_split(DEPDATAM, prop = 4/5, strata = DEPTot, breaks = 4, pool = 0.1)
...
TRGDTab<-table(train_data_GD$DEPTot)
TESTGDTab<-table(test_data_GD$DEPTot)
})
})
...
GD_rec <- recipe(DEPTot~ ., data = train_data_GD)%>% step_smote(DEPTot, over_ratio = 0.50)%>%step_zv(all_predictors())%>%step_nzv(all_predictors())%>%step_corr(all_predictors())%>%prep
...
Whole_data_GD_b<-bake(GD_rec, new_data=DEPDATAM)
...
options(scipen = 9999, digits=3, max.print=9999999, show.signif.stars=TRUE)
set.seed(123)
train_boot <- bootstraps(train_data_GD, strata = DEPTot)
```
...
options(scipen = 999, digits=3, max.print=999999, show.signif.stars=TRUE)
#Null_Model_Untuned
results_NF <- test_data_GD_b%>%select(DEPTot)%>% 
  bind_cols(Null_fit%>% 
...
results_NF %>% 
  conf_mat(truth = DEPTot, estimate = .pred_class)
#Visualise Results_NF
update_geom_defaults(geom = "rect", new = list(fill = "midnightblue", alpha = 0.7))
results_NF%>% 
  conf_mat(DEPTot,.pred_class) %>% 
  autoplot()

```

```{r}
ev_met1_NF<-ev_met1(results_NF,truth = DEPTot, estimate = .pred_class)
Acc_NF<-yardstick::accuracy(results_NF, DEPTot,.pred_class)
Rec_NF<-yardstick::recall(results_NF, DEPTot, .pred_class)
#Plot Roc_Curve
curve_NF <- results_NF %>% 
  roc_curve(DEPTot, .pred_No) %>% 
  autoplot
curve_NF
```
...
```{r}
ev_met1_RF<-ev_met1(results_RF,truth = DEPTot, estimate = .pred_class)
Rec_RF<-yardstick::recall(results_RF, DEPTot, .pred_class)
ACC_RF<-yardstick::accuracy(results_RF, DEPTot, .pred_class)
#Plot Roc_Curve
curve_RF <- results_RF %>% 
  roc_curve(DEPTot, .pred_No) %>% 
  autoplot
...
```
```{r}
results_RFW <- Whole_data_GD_b %>% select(DEPTot)%>% 
  bind_cols(RF_fit %>% 
...
```{r}
ev_met1_RFW<-ev_met1(results_RFW,truth = DEPTot, estimate = .pred_class)
Rec_RFW<-yardstick::recall(results_RFW, DEPTot, .pred_class)
ACC_RFW<-yardstick::accuracy(results_RFW, DEPTot, .pred_class)
#Plot Roc_Curve
...
Logistic Regression
```{r}
results_LR <- test_data_GD_b %>% select(DEPTot) %>% 
...
```{r}
ev_met1_LR<-ev_met1(results_LR,truth = DEPTot, estimate = .pred_class)
Rec_LR<-yardstick::recall(results_LR, DEPTot, .pred_class)
ACC_LR<-yardstick::accuracy(results_LR, DEPTot, .pred_class)
#Plot Roc_Curve
curve_LR <- results_LR %>% 
  roc_curve(DEPTot, .pred_No) %>% 
  autoplot
...
```
```{r}
results_LRW <- Whole_data_GD_b %>% select(DEPTot)%>% 
  bind_cols(LogReg_fit %>% 
              predict(new_data = Whole_data_GD_b))%>% 
  bind_cols(LogReg_fit%>% 
             predict(new_data = Whole_data_GD_b, type = "prob"))
...
```{r}
ev_met1_LRW<-ev_met1(results_LRW,truth = DEPTot, estimate = .pred_class)
Rec_LRW<-yardstick::recall(results_LRW, DEPTot, .pred_class)
ACC_LRW<-yardstick::accuracy(results_LRW, DEPTot, .pred_class)
#Plot Roc_Curve
...
Lasso
```{r}
results_Lasso<-test_data_GD_b %>% select(DEPTot) %>% 
  bind_cols( Lasso_fit%>% 
              predict(new_data = test_data_GD_b)) %>% 
  bind_cols( Lasso_fit%>% 
             predict(new_data = test_data_GD_b, type = "prob"))
...
```
```{r}
ev_met1_Lasso<-ev_met1(results_Lasso,truth = DEPTot, estimate = .pred_class)
ACC_Lasso<-yardstick::accuracy(results_Lasso, DEPTot,.pred_class)
Rec_Lasso<-yardstick::recall(results_Lasso, DEPTot, .pred_class)
#Plot Roc_Curve
...
```{r}
results_NB <- test_data_GD_b %>% select(DEPTot) %>% 
 bind_cols(NB_fit %>% 
             predict(new_data = test_data_GD_b)) %>% 
  bind_cols( NB_fit%>% 
              predict(new_data = test_data_GD_b, type = "prob"))
...
```{r}
ev_met1_NB<-ev_met1(results_NB,truth = DEPTot, estimate = .pred_class)
Rec_NB<-yardstick::recall(results_NB, DEPTot, .pred_class)
ACC_NB<-yardstick::accuracy(results_NB, DEPTot, .pred_class)
#Plot Roc_Curve
curve_NB <- results_NB %>% 
  roc_curve(DEPTot, .pred_No) %>% 
  autoplot
...
```{r}
results_Kern<-test_data_GD_b %>% select(DEPTot) %>% 
...
```{r}
ev_met1_Kern<-ev_met1(results_Kern,truth = DEPTot, estimate = .pred_class)
Rec_Kern<-yardstick::recall(results_Kern, DEPTot, .pred_class)
ACC_Kern<-yardstick::accuracy(results_Kern, DEPTot, .pred_class)
#Plot Roc_Curve
...
results_tuned_xgb<-test_data_GD_b %>% select(DEPTot) %>% 
...
warning=FALSE
options(scipen = 999, digits=3, max.print=999999, show.signif.stars=TRUE)
ev_met1_Tunxgb<-ev_met1(results_tuned_xgb,truth = DEPTot, estimate = .pred_class)
Rec_Txgb<-yardstick::recall(results_tuned_xgb, DEPTot, .pred_class)
ACC_Txgb<-yardstick::accuracy(results_tuned_xgb, DEPTot, .pred_class)
...
options(scipen = 999, digits=3, max.print=999999, show.signif.stars=TRUE)
#Test the tuned NB on the test data
results_tuned_LR<-test_data_GD_b %>% select(DEPTot) %>% 
...
warning=FALSE
options(scipen = 999, digits=3, max.print=999999, show.signif.stars=TRUE)
ev_met1_TunLR<-ev_met1(results_tuned_LR,truth = DEPTot, estimate = .pred_class)
Rec_TLR<-yardstick::recall(results_tuned_LR, DEPTot, .pred_class)
ACC_TLR<-yardstick::accuracy(results_tuned_LR, DEPTot, .pred_class)
...
options(scipen = 999, digits=3, max.print=999999, show.signif.stars=TRUE)
#Test the tuned NB on the test data
results_tuned_NB<-test_data_GD_b %>% select(DEPTot) %>% 
...
warning=FALSE
options(scipen = 999, digits=3, max.print=999999, show.signif.stars=TRUE)
ev_met1_TunNB<-ev_met1(results_tuned_NB,truth = DEPTot, estimate = .pred_class)
Rec_TNB<-yardstick::recall(results_tuned_NB, DEPTot, .pred_class)
ACC_TNB<-yardstick::accuracy(results_tuned_NB, DEPTot, .pred_class)
...
results_tuned_RF<-test_data_GD_b %>% select(DEPTot) %>% 
...
auc_RF_tuned <- results_tuned_RF %>% 
  roc_auc(DEPTot, .pred_No)
ev_met1_TunRF<-ev_met1(results_tuned_RF,truth = DEPTot, estimate = .pred_class)
Rec_TRF<-yardstick::recall(results_tuned_RF, DEPTot, .pred_class)
ACC_TRF<-yardstick::accuracy(results_tuned_RF, DEPTot, .pred_class)
...
results_tuned_Lasso<-test_data_GD_b%>% select(DEPTot) %>% 
...
```{r}
ev_met1_TunLasso<-ev_met1(results_tuned_Lasso,truth = DEPTot, estimate = .pred_class)
Rec_TLasso<-yardstick::recall(results_tuned_Lasso, DEPTot, .pred_class)
ACC_TLasso<-yardstick::accuracy(results_tuned_Lasso, DEPTot, .pred_class)
...
warning=FALSE
options(scipen = 999, digits=3, max.print=999999, show.signif.stars=TRUE)
results_tuned_knn<-test_data_GD_b%>% select(DEPTot) %>% 
...
```{r}
ev_met1_Tunknn<-ev_met1(results_tuned_knn,truth = DEPTot, estimate = .pred_class)
Rec_Tknn<-yardstick::recall(results_tuned_knn, DEPTot, .pred_class)
ACC_Tknn<-yardstick::accuracy(results_tuned_knn, DEPTot, .pred_class)
