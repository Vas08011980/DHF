...
IGDD<-IGDD%>%mutate(IGDTTotal=case_when(IGDTTotal>4~1,
                                      IGDTTotal<5~0,
                                      TRUE~NA_real_))
IGDD<-IGDD %>% 
...
set.seed(123)
GDTDiag$GDDiag=as_factor(GDTDiag$GDDiag)
GD_rec <- recipe(GDDiag~ ., data = train_data_GD)%>% step_smote(GDDiag, over_ratio = 0.50)%>%step_center(all_predictors())%>%step_scale(all_predictors())%>%step_normalize(all_numeric_predictors())%>%step_zv(all_predictors())%>%step_nzv(all_predictors())%>%step_corr(all_predictors())%>%prep()
