...
DATAAIGD<-DataM[c("Age_W1", "Yearsofplay_W1", "GDTTotal", "IMTotal", "IDTotal", "COMPTotal", "PETotal1", "PQTotal", "FTotal")]
...
GDTD<-GDTD%>%mutate(GDTTotal=case_when(GDTTotal>2~1,
                                      GDTTotal<3~0,
                                      TRUE~NA_real_))
GDTD<-GDTD %>% 
...
IGDD<-IGDD %>% 
  mutate(IGDTTotal = case_when(IGDTTotal == 1 ~ "Yes",
                             IGDTTotal == 0 ~ "No"))
  GDTDiag<-GDTD%>%setNames(c("AGE", "Gameyears","GDDiag","IMTotal", "IDTotal", "COMPTotal", "PETotal", "PQTotal", "FTotal")) 
...
set.seed(123)
GDTDiag$GDDiag=as_factor(GDTDiag$GDDiag)
GD_rec <- recipe(GDDiag~ ., data = train_data_GD)%>% step_smote(GDDiag, over_ratio = 0.50)%>%step_center(all_predictors())%>%step_normalize(all_numeric_predictors())%>%prep()
